...
import type { CreateOperatingPhaseInput } from "../../../hooks/use-operating-phase-schemas"
...
interface FormStepProps {
  form: UseFormReturn<CreateOperatingPhaseInput>
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-5 overflow-y-auto overscroll-contain px-4 py-4 sm:px-6 sm:py-5">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {(["startTime", "endTime"] as const).map((name) => (
            <Field key={name}>
              <FieldLabel htmlFor={`create-phase-${name}`}>
                {t(`form.${name}Label`)}
              </FieldLabel>
              <Input
