/**
 * Official Lunar-Based Variable Holidays for Iran (هجری شمسی)
 * Source: شورای مرکز تقویم مؤسسه ژئوفیزیک دانشگاه تهران
 * Description: تعطیلات رسمی متغیر قمری تقویم هجری شمسی ایران
 * Years Covered: 1400, 1401, 1402, 1403, 1404, 1405, 1406
 */

export interface LunarHolidayDefinition {
  month: number
  day: number
  titleFa: string
  titleEn: string
}

export const OFFICIAL_LUNAR_HOLIDAYS_METADATA = {
  source: "شورای مرکز تقویم مؤسسه ژئوفیزیک دانشگاه تهران",
  description: "تعطیلات رسمی متغیر قمری تقویم هجری شمسی ایران",
  yearsCovered: [1400, 1401, 1402, 1403, 1404, 1405, 1406],
} as const

export const OFFICIAL_LUNAR_HOLIDAYS: Record<number, LunarHolidayDefinition[]> =
  {
    "1400": [
      {
        month: 1,
        day: 9,
        titleFa: "ولادت حضرت قائم (عج) و نیمه شعبان",
        titleEn: "Mid-Sha'ban",
      },
      {
        month: 2,
        day: 14,
        titleFa: "شهادت حضرت علی (ع)",
        titleEn: "Martyrdom of Imam Ali",
      },
      {
        month: 2,
        day: 23,
        titleFa: "عید سعید فطر",
        titleEn: "Eid al-Fitr",
      },
      {
        month: 2,
        day: 24,
        titleFa: "تعطیل به مناسبت عید سعید فطر",
        titleEn: "Eid al-Fitr Holiday",
      },
      {
        month: 3,
        day: 16,
        titleFa: "شهادت امام جعفر صادق (ع)",
        titleEn: "Martyrdom of Imam Jafar Sadiq",
      },
      {
        month: 4,
        day: 30,
        titleFa: "عید سعید قربان",
        titleEn: "Eid al-Adha",
      },
      {
        month: 5,
        day: 7,
        titleFa: "عید سعید غدیر خم",
        titleEn: "Eid al-Ghadir",
      },
      {
        month: 5,
        day: 27,
        titleFa: "تاسوعای حسینی",
        titleEn: "Tasu'a",
      },
      {
        month: 5,
        day: 28,
        titleFa: "عاشورای حسینی",
        titleEn: "Ashura",
      },
      {
        month: 7,
        day: 5,
        titleFa: "اربعین حسینی",
        titleEn: "Arbaeen",
      },
      {
        month: 7,
        day: 13,
        titleFa: "رحلت رسول اکرم و شهادت امام حسن مجتبی (ع)",
        titleEn: "Demise of Prophet & Martyrdom of Imam Hassan",
      },
      {
        month: 7,
        day: 15,
        titleFa: "شهادت امام رضا (ع)",
        titleEn: "Martyrdom of Imam Reza",
      },
      {
        month: 7,
        day: 23,
        titleFa: "شهادت امام حسن عسکری (ع)",
        titleEn: "Martyrdom of Imam Hassan Askari",
      },
      {
        month: 8,
        day: 2,
        titleFa: "ولادت رسول اکرم و امام جعفر صادق (ع)",
        titleEn: "Birthday of Prophet & Imam Jafar Sadiq",
      },
      {
        month: 10,
        day: 16,
        titleFa: "شهادت حضرت فاطمه زهرا (س)",
        titleEn: "Martyrdom of Hazrat Fatemeh",
      },
      {
        month: 11,
        day: 26,
        titleFa: "ولادت حضرت علی (ع)",
        titleEn: "Birthday of Imam Ali",
      },
      {
        month: 12,
        day: 10,
        titleFa: "مبعث حضرت رسول اکرم (ص)",
        titleEn: "Mab'as",
      },
      {
        month: 12,
        day: 27,
        titleFa: "ولادت حضرت قائم (عج) و نیمه شعبان",
        titleEn: "Mid-Sha'ban",
      },
    ],
    "1401": [
      {
        month: 2,
        day: 3,
        titleFa: "شهادت حضرت علی (ع)",
        titleEn: "Martyrdom of Imam Ali",
      },
      {
        month: 2,
        day: 13,
        titleFa: "عید سعید فطر",
        titleEn: "Eid al-Fitr",
      },
      {
        month: 2,
        day: 14,
        titleFa: "تعطیل به مناسبت عید سعید فطر",
        titleEn: "Eid al-Fitr Holiday",
      },
      {
        month: 3,
        day: 6,
        titleFa: "شهادت امام جعفر صادق (ع)",
        titleEn: "Martyrdom of Imam Jafar Sadiq",
      },
      {
        month: 4,
        day: 19,
        titleFa: "عید سعید قربان",
        titleEn: "Eid al-Adha",
      },
      {
        month: 4,
        day: 27,
        titleFa: "عید سعید غدیر خم",
        titleEn: "Eid al-Ghadir",
      },
      {
        month: 5,
        day: 16,
        titleFa: "تاسوعای حسینی",
        titleEn: "Tasu'a",
      },
      {
        month: 5,
        day: 17,
        titleFa: "عاشورای حسینی",
        titleEn: "Ashura",
      },
      {
        month: 6,
        day: 26,
        titleFa: "اربعین حسینی",
        titleEn: "Arbaeen",
      },
      {
        month: 7,
        day: 3,
        titleFa: "رحلت رسول اکرم و شهادت امام حسن مجتبی (ع)",
        titleEn: "Demise of Prophet & Martyrdom of Imam Hassan",
      },
      {
        month: 7,
        day: 5,
        titleFa: "شهادت امام رضا (ع)",
        titleEn: "Martyrdom of Imam Reza",
      },
      {
        month: 7,
        day: 13,
        titleFa: "شهادت امام حسن عسکری (ع)",
        titleEn: "Martyrdom of Imam Hassan Askari",
      },
      {
        month: 7,
        day: 22,
        titleFa: "ولادت رسول اکرم و امام جعفر صادق (ع)",
        titleEn: "Birthday of Prophet & Imam Jafar Sadiq",
      },
      {
        month: 10,
        day: 6,
        titleFa: "شهادت حضرت فاطمه زهرا (س)",
        titleEn: "Martyrdom of Hazrat Fatemeh",
      },
      {
        month: 11,
        day: 15,
        titleFa: "ولادت حضرت علی (ع)",
        titleEn: "Birthday of Imam Ali",
      },
      {
        month: 11,
        day: 29,
        titleFa: "مبعث حضرت رسول اکرم (ص)",
        titleEn: "Mab'as",
      },
      {
        month: 12,
        day: 17,
        titleFa: "ولادت حضرت قائم (عج) و نیمه شعبان",
        titleEn: "Mid-Sha'ban",
      },
    ],
    "1402": [
      {
        month: 1,
        day: 23,
        titleFa: "شهادت حضرت علی (ع)",
        titleEn: "Martyrdom of Imam Ali",
      },
      {
        month: 2,
        day: 2,
        titleFa: "عید سعید فطر",
        titleEn: "Eid al-Fitr",
      },
      {
        month: 2,
        day: 3,
        titleFa: "تعطیل به مناسبت عید سعید فطر",
        titleEn: "Eid al-Fitr Holiday",
      },
      {
        month: 2,
        day: 26,
        titleFa: "شهادت امام جعفر صادق (ع)",
        titleEn: "Martyrdom of Imam Jafar Sadiq",
      },
      {
        month: 4,
        day: 8,
        titleFa: "عید سعید قربان",
        titleEn: "Eid al-Adha",
      },
      {
        month: 4,
        day: 16,
        titleFa: "عید سعید غدیر خم",
        titleEn: "Eid al-Ghadir",
      },
      {
        month: 5,
        day: 5,
        titleFa: "تاسوعای حسینی",
        titleEn: "Tasu'a",
      },
      {
        month: 5,
        day: 6,
        titleFa: "عاشورای حسینی",
        titleEn: "Ashura",
      },
      {
        month: 6,
        day: 15,
        titleFa: "اربعین حسینی",
        titleEn: "Arbaeen",
      },
      {
        month: 6,
        day: 23,
        titleFa: "رحلت رسول اکرم و شهادت امام حسن مجتبی (ع)",
        titleEn: "Demise of Prophet & Martyrdom of Imam Hassan",
      },
      {
        month: 6,
        day: 25,
        titleFa: "شهادت امام رضا (ع)",
        titleEn: "Martyrdom of Imam Reza",
      },
      {
        month: 7,
        day: 2,
        titleFa: "شهادت امام حسن عسکری (ع)",
        titleEn: "Martyrdom of Imam Hassan Askari",
      },
      {
        month: 7,
        day: 11,
        titleFa: "ولادت رسول اکرم و امام جعفر صادق (ع)",
        titleEn: "Birthday of Prophet & Imam Jafar Sadiq",
      },
      {
        month: 9,
        day: 26,
        titleFa: "شهادت حضرت فاطمه زهرا (س)",
        titleEn: "Martyrdom of Hazrat Fatemeh",
      },
      {
        month: 11,
        day: 5,
        titleFa: "ولادت حضرت علی (ع)",
        titleEn: "Birthday of Imam Ali",
      },
      {
        month: 11,
        day: 19,
        titleFa: "مبعث حضرت رسول اکرم (ص)",
        titleEn: "Mab'as",
      },
      {
        month: 12,
        day: 6,
        titleFa: "ولادت حضرت قائم (عج) و نیمه شعبان",
        titleEn: "Mid-Sha'ban",
      },
    ],
    "1403": [
      {
        month: 1,
        day: 13,
        titleFa: "شهادت حضرت علی (ع)",
        titleEn: "Martyrdom of Imam Ali",
      },
      {
        month: 1,
        day: 22,
        titleFa: "عید سعید فطر",
        titleEn: "Eid al-Fitr",
      },
      {
        month: 1,
        day: 23,
        titleFa: "تعطیل به مناسبت عید سعید فطر",
        titleEn: "Eid al-Fitr Holiday",
      },
      {
        month: 2,
        day: 15,
        titleFa: "شهادت امام جعفر صادق (ع)",
        titleEn: "Martyrdom of Imam Jafar Sadiq",
      },
      {
        month: 3,
        day: 28,
        titleFa: "عید سعید قربان",
        titleEn: "Eid al-Adha",
      },
      {
        month: 4,
        day: 5,
        titleFa: "عید سعید غدیر خم",
        titleEn: "Eid al-Ghadir",
      },
      {
        month: 4,
        day: 25,
        titleFa: "تاسوعای حسینی",
        titleEn: "Tasu'a",
      },
      {
        month: 4,
        day: 26,
        titleFa: "عاشورای حسینی",
        titleEn: "Ashura",
      },
      {
        month: 6,
        day: 4,
        titleFa: "اربعین حسینی",
        titleEn: "Arbaeen",
      },
      {
        month: 6,
        day: 12,
        titleFa: "رحلت رسول اکرم و شهادت امام حسن مجتبی (ع)",
        titleEn: "Demise of Prophet & Martyrdom of Imam Hassan",
      },
      {
        month: 6,
        day: 14,
        titleFa: "شهادت امام رضا (ع)",
        titleEn: "Martyrdom of Imam Reza",
      },
      {
        month: 6,
        day: 22,
        titleFa: "شهادت امام حسن عسکری (ع)",
        titleEn: "Martyrdom of Imam Hassan Askari",
      },
      {
        month: 6,
        day: 31,
        titleFa: "ولادت رسول اکرم و امام جعفر صادق (ع)",
        titleEn: "Birthday of Prophet & Imam Jafar Sadiq",
      },
      {
        month: 9,
        day: 15,
        titleFa: "شهادت حضرت فاطمه زهرا (س)",
        titleEn: "Martyrdom of Hazrat Fatemeh",
      },
      {
        month: 10,
        day: 25,
        titleFa: "ولادت حضرت علی (ع)",
        titleEn: "Birthday of Imam Ali",
      },
      {
        month: 11,
        day: 9,
        titleFa: "مبعث حضرت رسول اکرم (ص)",
        titleEn: "Mab'as",
      },
      {
        month: 11,
        day: 26,
        titleFa: "ولادت حضرت قائم (عج) و نیمه شعبان",
        titleEn: "Mid-Sha'ban",
      },
    ],
    "1404": [
      {
        month: 1,
        day: 2,
        titleFa: "شهادت حضرت علی (ع)",
        titleEn: "Martyrdom of Imam Ali",
      },
      {
        month: 1,
        day: 11,
        titleFa: "عید سعید فطر",
        titleEn: "Eid al-Fitr",
      },
      {
        month: 1,
        day: 12,
        titleFa: "تعطیل به مناسبت عید سعید فطر",
        titleEn: "Eid al-Fitr Holiday",
      },
      {
        month: 2,
        day: 4,
        titleFa: "شهادت امام جعفر صادق (ع)",
        titleEn: "Martyrdom of Imam Jafar Sadiq",
      },
      {
        month: 3,
        day: 16,
        titleFa: "عید سعید قربان",
        titleEn: "Eid al-Adha",
      },
      {
        month: 3,
        day: 24,
        titleFa: "عید سعید غدیر خم",
        titleEn: "Eid al-Ghadir",
      },
      {
        month: 4,
        day: 14,
        titleFa: "تاسوعای حسینی",
        titleEn: "Tasu'a",
      },
      {
        month: 4,
        day: 15,
        titleFa: "عاشورای حسینی",
        titleEn: "Ashura",
      },
      {
        month: 5,
        day: 23,
        titleFa: "اربعین حسینی",
        titleEn: "Arbaeen",
      },
      {
        month: 5,
        day: 31,
        titleFa: "رحلت رسول اکرم و شهادت امام حسن مجتبی (ع)",
        titleEn: "Demise of Prophet & Martyrdom of Imam Hassan",
      },
      {
        month: 6,
        day: 2,
        titleFa: "شهادت امام رضا (ع)",
        titleEn: "Martyrdom of Imam Reza",
      },
      {
        month: 6,
        day: 10,
        titleFa: "شهادت امام حسن عسکری (ع)",
        titleEn: "Martyrdom of Imam Hassan Askari",
      },
      {
        month: 6,
        day: 19,
        titleFa: "ولادت رسول اکرم و امام جعفر صادق (ع)",
        titleEn: "Birthday of Prophet & Imam Jafar Sadiq",
      },
      {
        month: 9,
        day: 3,
        titleFa: "شهادت حضرت فاطمه زهرا (س)",
        titleEn: "Martyrdom of Hazrat Fatemeh",
      },
      {
        month: 10,
        day: 13,
        titleFa: "ولادت حضرت علی (ع)",
        titleEn: "Birthday of Imam Ali",
      },
      {
        month: 10,
        day: 27,
        titleFa: "مبعث حضرت رسول اکرم (ص)",
        titleEn: "Mab'as",
      },
      {
        month: 11,
        day: 15,
        titleFa: "ولادت حضرت قائم (عج) و نیمه شعبان",
        titleEn: "Mid-Sha'ban",
      },
      {
        month: 12,
        day: 20,
        titleFa: "شهادت حضرت علی (ع)",
        titleEn: "Martyrdom of Imam Ali",
      },
    ],
    "1405": [
      {
        month: 1,
        day: 1,
        titleFa: "عید سعید فطر",
        titleEn: "Eid al-Fitr",
      },
      {
        month: 1,
        day: 2,
        titleFa: "تعطیل به مناسبت عید سعید فطر",
        titleEn: "Eid al-Fitr Holiday",
      },
      {
        month: 1,
        day: 25,
        titleFa: "شهادت امام جعفر صادق (ع)",
        titleEn: "Martyrdom of Imam Jafar Sadiq",
      },
      {
        month: 3,
        day: 7,
        titleFa: "عید سعید قربان",
        titleEn: "Eid al-Adha",
      },
      {
        month: 3,
        day: 15,
        titleFa: "عید سعید غدیر خم",
        titleEn: "Eid al-Ghadir",
      },
      {
        month: 4,
        day: 4,
        titleFa: "تاسوعای حسینی",
        titleEn: "Tasu'a",
      },
      {
        month: 4,
        day: 5,
        titleFa: "عاشورای حسینی",
        titleEn: "Ashura",
      },
      {
        month: 5,
        day: 13,
        titleFa: "اربعین حسینی",
        titleEn: "Arbaeen",
      },
      {
        month: 5,
        day: 21,
        titleFa: "رحلت رسول اکرم و شهادت امام حسن مجتبی (ع)",
        titleEn: "Demise of Prophet & Martyrdom of Imam Hassan",
      },
      {
        month: 5,
        day: 23,
        titleFa: "شهادت امام رضا (ع)",
        titleEn: "Martyrdom of Imam Reza",
      },
      {
        month: 5,
        day: 31,
        titleFa: "شهادت امام حسن عسکری (ع)",
        titleEn: "Martyrdom of Imam Hassan Askari",
      },
      {
        month: 6,
        day: 9,
        titleFa: "ولادت رسول اکرم و امام جعفر صادق (ع)",
        titleEn: "Birthday of Prophet & Imam Jafar Sadiq",
      },
      {
        month: 8,
        day: 23,
        titleFa: "شهادت حضرت فاطمه زهرا (س)",
        titleEn: "Martyrdom of Hazrat Fatemeh",
      },
      {
        month: 10,
        day: 2,
        titleFa: "ولادت حضرت علی (ع)",
        titleEn: "Birthday of Imam Ali",
      },
      {
        month: 10,
        day: 16,
        titleFa: "مبعث حضرت رسول اکرم (ص)",
        titleEn: "Mab'as",
      },
      {
        month: 11,
        day: 4,
        titleFa: "ولادت حضرت قائم (عج) و نیمه شعبان",
        titleEn: "Mid-Sha'ban",
      },
      {
        month: 12,
        day: 10,
        titleFa: "شهادت حضرت علی (ع)",
        titleEn: "Martyrdom of Imam Ali",
      },
      {
        month: 12,
        day: 19,
        titleFa: "عید سعید فطر",
        titleEn: "Eid al-Fitr",
      },
      {
        month: 12,
        day: 20,
        titleFa: "تعطیل به مناسبت عید سعید فطر",
        titleEn: "Eid al-Fitr Holiday",
      },
    ],
    "1406": [
      {
        month: 1,
        day: 14,
        titleFa: "شهادت امام جعفر صادق (ع)",
        titleEn: "Martyrdom of Imam Jafar Sadiq",
      },
      {
        month: 2,
        day: 27,
        titleFa: "عید سعید قربان",
        titleEn: "Eid al-Adha",
      },
      {
        month: 3,
        day: 4,
        titleFa: "عید سعید غدیر خم",
        titleEn: "Eid al-Ghadir",
      },
      {
        month: 3,
        day: 25,
        titleFa: "تاسوعای حسینی",
        titleEn: "Tasu'a",
      },
      {
        month: 3,
        day: 26,
        titleFa: "عاشورای حسینی",
        titleEn: "Ashura",
      },
      {
        month: 5,
        day: 3,
        titleFa: "اربعین حسینی",
        titleEn: "Arbaeen",
      },
      {
        month: 5,
        day: 11,
        titleFa: "رحلت رسول اکرم و شهادت امام حسن مجتبی (ع)",
        titleEn: "Demise of Prophet & Martyrdom of Imam Hassan",
      },
      {
        month: 5,
        day: 12,
        titleFa: "شهادت امام رضا (ع)",
        titleEn: "Martyrdom of Imam Reza",
      },
      {
        month: 5,
        day: 20,
        titleFa: "شهادت امام حسن عسکری (ع)",
        titleEn: "Martyrdom of Imam Hassan Askari",
      },
      {
        month: 5,
        day: 29,
        titleFa: "ولادت رسول اکرم و امام جعفر صادق (ع)",
        titleEn: "Birthday of Prophet & Imam Jafar Sadiq",
      },
      {
        month: 8,
        day: 12,
        titleFa: "شهادت حضرت فاطمه زهرا (س)",
        titleEn: "Martyrdom of Hazrat Fatemeh",
      },
      {
        month: 9,
        day: 21,
        titleFa: "ولادت حضرت علی (ع)",
        titleEn: "Birthday of Imam Ali",
      },
      {
        month: 10,
        day: 5,
        titleFa: "مبعث حضرت رسول اکرم (ص)",
        titleEn: "Mab'as",
      },
      {
        month: 10,
        day: 23,
        titleFa: "ولادت حضرت قائم (عج) و نیمه شعبان",
        titleEn: "Mid-Sha'ban",
      },
      {
        month: 11,
        day: 29,
        titleFa: "شهادت حضرت علی (ع)",
        titleEn: "Martyrdom of Imam Ali",
      },
      {
        month: 12,
        day: 8,
        titleFa: "عید سعید فطر",
        titleEn: "Eid al-Fitr",
      },
      {
        month: 12,
        day: 9,
        titleFa: "تعطیل به مناسبت عید سعید فطر",
        titleEn: "Eid al-Fitr Holiday",
      },
    ],
  }
